 ...
 */
export function buildEdmx(schema: ODataSchemaConfig, basePath: string): string {
  const doc = create({ version: '1.0', encoding: 'utf-8' })
    ...
    .ele('edmx:DataServices', {
      'm:DataServiceVersion': ODATA_VERSION,
 ...
 */
function buildFunctionImport(
  parent: ReturnType<typeof create>,
  name: string,
  func: FunctionImportDefinition,
  schema: ODataSchemaConfig
): void {
  ...
  if (func.parameters) {
    for (const [paramName, param] of Object.entries(func.parameters)) {
      const paramAttrs: Record<string, string> = {
        Name: paramName,
        Type: param.type,
      };
 ...
 */
function formatMultiplicity(mult: Multiplicity): string {
  switch (mult) {
    case '0..1':
      return '0..1';
    case '1':
 ...
 */
function formatReturnType(returnType: string, namespace: string): string {
