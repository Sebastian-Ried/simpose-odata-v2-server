...
import { buildEdmx } from './edmx-builder';
import { Model, ModelStatic } from 'sequelize';
import { sequelizeToEdmType } from './type-mapping';
...
export class MetadataGenerator {
  ...
  private cachedEdmx: string | null = null;
  private cachedEnrichedSchema: ODataSchemaConfig | null = null;
  ...
  generateEdmx(): string {
    ...
    this.cachedEdmx = buildEdmx(enrichedSchema, this.basePath);
    return this.cachedEdmx;
  }
   ...
   */
  private enrichSchemaFromModels(): ODataSchemaConfig {
    const enriched: ODataSchemaConfig = {
      ...this.schema,
    ...
    for (const [entityName, entity] of Object.entries(enriched.entities)) {
      ...
      for (const [attrName, attr] of Object.entries(attributes)) {
        if (!enrichedProperties[attrName]) {
          ...
          const typeObj = attr.type as any;
          if (
            typeObj.constructor?.name === 'STRING' ||
            typeObj.key === 'STRING'
          ) {
            if (typeObj._length) {
          ...
          if (
            typeObj.constructor?.name === 'DECIMAL' ||
            typeObj.key === 'DECIMAL'
          ) {
      ...
      for (const [assocName, assoc] of Object.entries(associations) as [string, any][]) {
        if (!enrichedNavProps[assocName]) {
          const targetEntityName = this.findEntityByModel(
            assoc.target.name,
            enriched
          );
          if (targetEntityName) {
            ...
            if (!enriched.associations[relationshipName]) {
              enriched.associations[relationshipName] = this.createAssociationFromSequelize(
                entityName,
                targetEntityName,
                assoc
              );
   ...
   */
  private getMultiplicityFromAssociation(assoc: any): '0..1' | '1' | '*' {
  ...
  private createAssociationFromSequelize(
    ...
    targetEntity: string,
    assoc: any
  ): { ends: [{ entity: string; multiplicity: '0..1' | '1' | '*' }, { entity: string; multiplicity: '0..1' | '1' | '*' }]; referentialConstraint?: any } {
    const assocType = assoc.associationType;
    let sourceMultiplicity: '0..1' | '1' | '*' = '*';
    let targetMultiplicity: '0..1' | '1' | '*' = '*';
    ...
    const association: { ends: [{ entity: string; multiplicity: '0..1' | '1' | '*' }, { entity: string; multiplicity: '0..1' | '1' | '*' }]; referentialConstraint?: any } = {
      ends: [
