...
export function parseBatchRequest(
  ...
  const parts: (BatchPart | BatchChangeset)[] = [];
 ...
 */
function parseBatchSection(section: string): BatchPart | BatchChangeset | null {
 ...
 * Parse a changeset
 */
function parseChangeset(body: string, boundary: string): BatchChangeset {
 ...
 */
function parseIndividualRequest(
  body: string,
...
export function buildBatchResponse(
  parts: (BatchResponsePart | BatchResponsePart[])[],
  boundary: string
): string {
  const lines: string[] = [];
 ...
 */
function buildResponsePart(part: BatchResponsePart): string[] {
  const lines: string[] = [];
