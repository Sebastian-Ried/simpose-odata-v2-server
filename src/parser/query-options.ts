import { ParsedQuery, ExpandOption, OrderByOption } from '../config/types';
import { parseFilter } from './filter-parser';
import { MAX_PAGE_SIZE } from '../config/defaults';
 ...
 */
export function parseQueryOptions(query: Record<string, string | undefined>): ParsedQuery {
  const parsed: ParsedQuery = {};

  // $filter
  if (query['$filter']) {
    try {
      parsed.$filter = parseFilter(query['$filter']);
    } catch (e) {
      throw new QueryOptionError('$filter', (e as Error).message);
    }
  }
  ...
  if (query['$expand']) {
    try {
      ...
      throw new QueryOptionError('$expand', (e as Error).message);
    }
  }
  ...
  if (query['$top']) {
    ...
    if (top > MAX_PAGE_SIZE) {
      throw new QueryOptionError('$top', `exceeds maximum of ${MAX_PAGE_SIZE}`);
    }
    parsed.$top = top;
  }
  ...
  if (query['$inlinecount']) {
    ...
    parsed.$inlinecount = value as 'allpages' | 'none';
  }
  ...
  if (query['$format']) {
    ...
    parsed.$format = format as 'json' | 'atom';
  }
 ...
 */
function parseExpand(expand: string): ExpandOption[] {
 ...
 */
function splitExpandItems(expand: string): string[] {
  const items: string[] = [];
  let current = '';
  let depth = 0;
 ...
 */
function parseExpandItem(item: string): ExpandOption | null {
  if (!item) return null;
  ...
  if (item.includes('/')) {
    ...
    let current: ExpandOption | null = null;
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
      ...
      const newOption: ExpandOption = { property: part };
      if (current) {
 ...
 */
function parseExpandOptions(property: string, optionsStr: string): ExpandOption {
  const option: ExpandOption = { property };
 ...
 */
function parseOrderBy(orderby: string): OrderByOption[] {
  return orderby
    .split(',')
    .map((item) => {
...
export function validateQueryOptions(
  options: ParsedQuery,
  entityName: string,
  schema: import('../config/types').ODataSchemaConfig
): ParsedQuery {
  ...
  if (options.$select) {
    const invalidProps = options.$select.filter(
      (prop) => !entity.properties[prop] && !entity.navigationProperties?.[prop]
    );
    if (invalidProps.length > 0) {
      throw new QueryOptionError(
        '$select',
        `Unknown properties: ${invalidProps.join(', ')}`
      );
    }
  }
  ...
  if (options.$expand) {
    for (const expand of options.$expand) {
      if (!entity.navigationProperties?.[expand.property]) {
        throw new QueryOptionError(
          '$expand',
          `Unknown navigation property: ${expand.property}`
        );
      }
    }
  }
  ...
  if (options.$orderby) {
    for (const order of options.$orderby) {
      ...
      if (!entity.properties[baseProp] && !entity.navigationProperties?.[baseProp]) {
        throw new QueryOptionError(
          '$orderby',
          `Unknown property: ${order.property}`
        );
      }
    }
  }
