...
function parseEntityKeys(
  ...
  if (Object.keys(keys).length === 1 && keys['']) {
    ...
    const value = odataLiteralToValue(keys[''] as string, edmType);
    return { [primaryKey]: value };
  }

  // Parse typed values
  const typedKeys: Record<string, unknown> = {};
  for (const [keyName, keyValue] of Object.entries(keys)) {
    const edmType = entity.properties[keyName]?.type;
    typedKeys[keyName] = odataLiteralToValue(keyValue as string, edmType);
  }
...
function parseKeyString(keyString: string): Record<string, unknown> {
  ...
  for (const pair of pairs) {
    ...
    if (eqIndex > 0) {
      ...
      let value: unknown = pair.slice(eqIndex + 1).trim();

      // Parse value
      if (typeof value === 'string') {
        value = odataLiteralToValue(value);
      }
 ...
 */
function getEntityNameFromSegment(
  segment: UriSegment,
  schema: ODataSchemaConfig
): string | null {
