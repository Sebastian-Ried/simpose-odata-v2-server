import { FilterExpression, ODataSchemaConfig, EntityDefinition } from '../config/types';
import { Op, WhereOptions, Sequelize, literal, fn, col, where } from 'sequelize';
...
function validatePropertyPath(
  ...
  let currentEntity: EntityDefinition | undefined = schema.entities[entityName];
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    const navProp: { target: string; multiplicity?: string } | undefined = currentEntity!.navigationProperties?.[part];
    if (navProp) {
      if (isLast) {
      ...
      const targetEntity: EntityDefinition | undefined = schema.entities[navProp.target];
      if (!targetEntity) {
...
function translateNode(
  ...
  switch (node.type) {
    ...
    default:
      throw new Error(`Unknown filter node type: ${(node as any).type}`);
  }
}
...
function buildArithmeticExpression(
  ...
  schema: ODataSchemaConfig,
  sequelize: Sequelize
): { sql: string; hasColumn: boolean } {
  if (node.type === 'literal') {
    // Literal values - escape them properly
    const value = node.value as string | number | Date;
 ...
 */
function getArithmeticSqlOperator(operator: string): string {
  switch (operator) {
    case 'add': return '+';
    case 'sub': return '-';
    case 'mul': return '*';
...
function translateBinaryOp(
  ...
  if (isComparisonOperator(operator!)) {
    ...
    if (leftHasArithmetic || rightHasArithmetic) {
      ...
      return literal(`(${leftExpr.sql} ${compOp} ${rightExpr.sql})`);
    }
    ...
    if (left.type === 'function') {
      ...
      return where(leftValue, sqlOp, rightValue);
    }

    // Fallback for other cases
    return where(literal(String(leftValue)), getSqlOperator(operator!), rightValue);
  }
...
function translateUnaryOp(
  ...
  if (operator === 'not') {
    return {
      [Op.not]: translateNode(operand, entityName, schema, sequelize, models),
    };
  }
...
function translateFunction(
  ...
  const translatedArgs = args.map((arg) =>
    translateNode(arg, entityName, schema, sequelize)
  );
  ...
  switch (funcName) {
    ...
    case 'substringof': {
      ...
      if (args[1]?.type === 'property') {
        ...
        if (propName.includes('.')) {
          return { [`$${propName}$`]: { [Op.iLike]: `%${escapedNeedle}%` } };
        }
        return { [propName]: { [Op.iLike]: `%${escapedNeedle}%` } };
      }
      const escapedNeedle = typeof needle === 'string' ? escapeLikePattern(needle) : String(needle);
      return where(col(String(haystack)), Op.iLike, `%${escapedNeedle}%`);
    }
    ...
    case 'startswith': {
      ...
      if (args[0]?.type === 'property') {
        ...
        if (propName.includes('.')) {
          return { [`$${propName}$`]: { [Op.iLike]: `${escapedValue}%` } };
        }
        return { [propName]: { [Op.iLike]: `${escapedValue}%` } };
      }
      const escapedValue = typeof value === 'string' ? escapeLikePattern(value) : String(value);
      return where(col(String(prop)), Op.iLike, `${escapedValue}%`);
    }
    ...
    case 'length': {
      ...
      if (args[0]?.type === 'property') {
        return fn('LENGTH', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'indexof': {
      ...
      if (args[0]?.type === 'property') {
        return literal(
          `INSTR(${getPropertyPath(args[0].name!)}, ${sequelize.escape(search)}) - 1`
        );
      }
      return literal(`INSTR(${prop}, ${sequelize.escape(search)}) - 1`);
    }

    case 'substring': {
      const [prop, start, length] = translatedArgs;
      // OData uses 0-based index
      const startPos = (start as number) + 1;
      if (args[0]?.type === 'property') {
        const propName = getPropertyPath(args[0].name!);
        if (length !== undefined) {
          return fn('SUBSTRING', col(propName), startPos, length);
        }
        return fn('SUBSTRING', col(propName), startPos);
      }
      if (length !== undefined) {
        return fn('SUBSTRING', prop, startPos, length);
      }
      return fn('SUBSTRING', prop, startPos);
    }

    case 'tolower': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('LOWER', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'toupper': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('UPPER', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'trim': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('TRIM', col(getPropertyPath(args[0].name!)));
      }
      return fn('TRIM', prop);
    }
    ...
    case 'replace': {
      ...
      if (args[0]?.type === 'property') {
        return fn(
          'REPLACE',
          col(getPropertyPath(args[0].name!)),
          search,
          replacement
        );
      }
      return fn('REPLACE', prop, search, replacement);
    }
    ...
    case 'year': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('YEAR', col(getPropertyPath(args[0].name!)));
      }
      return fn('YEAR', prop);
    }
    ...
    case 'month': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('MONTH', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'day': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('DAY', col(getPropertyPath(args[0].name!)));
      }
      return fn('DAY', prop);
    }
    ...
    case 'hour': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('HOUR', col(getPropertyPath(args[0].name!)));
      }
      return fn('HOUR', prop);
    }
    ...
    case 'minute': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('MINUTE', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'second': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('SECOND', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'round': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('ROUND', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'floor': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('FLOOR', col(getPropertyPath(args[0].name!)));
      }
    ...
    case 'ceiling': {
      const [prop] = translatedArgs;
      if (args[0]?.type === 'property') {
        return fn('CEILING', col(getPropertyPath(args[0].name!)));
      }
      return fn('CEILING', prop);
    }
 ...
 */
function getPropertyPath(odataPath: string): string {
 ...
 */
function resolveColumnPath(
  dottedPath: string,
  entityName: string,
  schema: ODataSchemaConfig,
  models?: Record<string, any>
): string {
  if (!models) return dottedPath;

  const parts = dottedPath.split('.');
  if (parts.length < 2) return dottedPath;
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    if (navProp) {
      ...
      if (targetEntity) {
        targetModelName = (targetEntity as any).model || navProp.target;
        currentEntity = targetEntity;
      }
    }
  }
 ...
 */
function getSequelizeOperator(operator: string): symbol {
  switch (operator) {
    case 'eq':
      return Op.eq;
    case 'ne':
