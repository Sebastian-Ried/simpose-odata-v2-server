/**
 * Seed data for Heidelberg places (PostGIS version)
 *
 * Real places worth visiting in Heidelberg, Germany.
 * Coordinates are provided in EPSG:4326 (WGS84/GPS) and transformed
 * to EPSG:25832 (UTM zone 32N) during seeding using PostGIS.
 */

import { Sequelize, QueryTypes, ModelStatic } from 'sequelize';
import { HeidelbergPlace } from './model';
...
/**
 * Transform GPS coordinates (EPSG:4326) to UTM coordinates (EPSG:25832)
 ...
 * Transforms GPS coordinates to EPSG:25832 during insertion
 */
export async function seedHeidelbergPlaces(
  model: ModelStatic<HeidelbergPlace>,
  sequelize: Sequelize
): Promise<void> {
  console.log('Seeding Heidelberg places (PostGIS)...');

  for (const place of heidelbergPlaces) {
    // Transform GPS coordinates to UTM (EPSG:25832)
    const location = await transformToUTM(sequelize, place.Latitude, place.Longitude);
  ...
  console.log(`✓ Seeded ${heidelbergPlaces.length} places with PostGIS coordinates`);
}
